/*
    ...
    - 객체 안에 여러가지 데이터/자료 포함시킬 수 있다. (라이브러리 제공할때 편리함)

    - 브라우저 세션과 쿠키
        f12[개발자도구] -> application탭 -> 1. Local storage 2. Session storage

        - JS 실행시점 : http(주소) 이용한 html 요청했을 때 -----> html열렸을 때(html에서 js를 포함하고 있기 때문에)
        - F5(새로고침) : 현재 HTTP(주소) 재요청 -> html -> js ( 메모리 초기화 )
            - js에 있는 메모리 영구 저장 ( 외부 이용 )
                1. DataBase( DB ) 2. 브라우저( 프로그램 ) 3. 자바( 서버프로그램 )
        
        브라우저 저장소 ( 보안 취약 )
        1. Local storage    객체 : 브라우저 꺼져도 데이터 유지 , 유저가 직접 삭제 , ( 활용 : 자동로그인 )
        2. Session storage  객체 : 프로그램(브라우저) 꺼지면 데이터 사라짐 ( 활용 : 검색엔진 , 임시데이터 )
            - 속성
