...
console.log( 영화API.boxOfficeResult ); // 자료 결과 정보 , 주간 박스오피스 정보
    // 영화API.boxOfficeResult :  {weeklyBoxOfficeList : 값 }

console.log( 영화API.boxOfficeResult.weeklyBoxOfficeList); // 주간 박스 오피스의 영화정보 목록
    // 영화API.boxOfficeResult.weeklyBoxOfficeList : [ 객체 , 객체 , 객체 , 객체 ]

console.log( 영화API.boxOfficeResult.weeklyBoxOfficeList[1].rank);  //2번째 영화정보의 순위
console.log( 영화API.boxOfficeResult.weeklyBoxOfficeList[1].movieNm);  //2번째 영화정보의 영화명
    // [ 객체[0] , 객체[1] , 객체[2] , 객체[3] ]
    // 객체[1] : { rank : 값 , movieNm : 값 }

// 2. HTML 출력 함수 : 실행조건 : 1. JS열렸을 때 : 어디에 무엇을 출력 판단하기
moviePrint();   // 함수실행 : js가 실행될 때 함수 1번 실행해주기.
function moviePrint(){  // 함수 정의/만들기
    ...
    for( let i = 0 ; i < 영화목록.length ; i++ ){
        let 영화 = 영화목록[i]; console.log( 영화 );        // 코드를 줄이기 위해 let 영화 변수를 준것임. 영화목록[i] 그대로 써도됨.

        // 무엇을 반복해서 누적 HTML 문자열 누적 대입
        무엇을 += `<tr>
                    <td> ${영화.rank}</td>
