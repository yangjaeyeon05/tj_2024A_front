/*
    ...
    - day14
        ...
        게시물목록 = [ 게시물객체{} , 게시물객체{} , 게시물객체{} ]
            - 여러 객체들을 식별할때 인덱스는 문제점이 있다. 인덱스는 절대인 식별자 역할 못함
            - 게시물 번호 만들어보자 = 삭제/수정되도 고정으로 사용하자
            
 */

let boardList = [
    { 번호 : 1 , 제목 :' 제목입니다' , 내용 : '내용입니다' , 비밀번호 : '1234' , 작성일 : '2024-05-24' , 조회수 : 3 },
    { 번호 : 2 , 제목 :' 두번째입니다' , 내용 : '또 내용입니다' , 비밀번호 : '4567' , 작성일 : '2024-05-25' , 조회수 : 0 },
];  // 배열 선언끝
...
function _create(){
     ...
     // let board =`${title},${content},${password},${date},${view}`; 배열형식으로 만든 변수

     // 게시물 번호 1증가 = 수정
     autoNum += 1;  // vs autoNum += 1; vs autoNum++
...
function _allRead(){
    ...
    for( let i = 0 ; i < boardList.length ; i++ ){
        ...
        html +=`<tr> 
                <td onclick="_read(${board.번호})"> ${board.번호} </td>
                <td> ${board.제목} </td> 
                <td> ${board.작성일} </td> 
                <td> ${board.조회수} </td>
                </tr>`
...
function _delete(번호){
    ...
    // 패스워드 검증 함수를 호출 후 반환값이 false이면 삭제함수 강제종료 
...
function findBoardIndex(번호){
     ...
     for( let i = 0 ; i < boardList.length ; i++ ){
        // 만약에 배열내 i번째 게시물의 번호와 내가 클릭한 번호와 같으면 findIndex에 대입하고 반복문 종료
