/*
    0. 키워드 : JS회사에서 미fl 만든 의미/기능 담긴 단어
    1. 세미클론 ; 
        - 컴퓨터에게 문장 마침 알림 역할
...
let 잘라낸차량번호 = 차량번호.split( '어' );       // 특정문자 기준으로 분리해서 배열 반환
console.log( 잘라낸차량번호 );                    // ['250', '7142']

// 예1 > 2명의 사원이름을 입력 받아 HTML(table)형식 출력(` `)하시오.

let 사원1 = prompt('사원1 사원명 : ' );             // 1. 사원 이름 입력받기 2.변수에 저장한다.
    console.log( 사원1 );
let 사원2 = prompt('사원2 사원명 : ' );             // 1. 사원 이름 입력받기 2.변수에 저장한다.
    console.log( 사원2 );
// 3. HTML 형식의 문자열을 저장하는 배열
    // 1. HTML에서 표현하고자하는 HTML마크업을 만들어서 복붙.
    // 2. ` ` 백틱 템플릿에서 변수를 호출 시 ${변수명}
let tableHTML = `<table>
                    <tr>
                        <th> 번호 </th> <th> 이름</th>
                    </tr>
...
// 4. HTML 문자열이 저장된 변수를 HTML에 출력하자
