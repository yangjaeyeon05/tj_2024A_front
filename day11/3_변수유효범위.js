...
function 등록( ){
    ...
    // let 배열 = [ ];     // 0.배열 선언 , 함수가 끝나면 해당 { }안에 있는 메모이 사라짐.
    // let 입력 = prompt( '성명' );    // 1. 입력받고 변수저장한다.
    ...
    // 입력받고 저장 후 input 공백으로 만들기
    document.querySelector('#성명').value = '';
}

// 함수 사용하는 이유
    // 1. 반복되는 코드를 한번에 정의하고 필요에 따라 호출해서 반복 작업을 피할 수 있다. (코드 양 줄이기)
    // 2. 긴 프로그램을 기능별로 함수를 구성해서 여러 함수로 모듈화(조각/레고)로 전체 코드의 가독성(읽기) 좋다
    // 3. 기능별(함수별)로 수정이 편하고 유지보수 쉽다.
    // 4. 매개변수/인수에 따른 서로 다른 결과물  ex) x=3일떄와 x=5일때 서로 다른 결과
    // 5. { } 지역변수 사용하기 때문에 메모리 효율성이 좋다. 
        // 함수 실행될 때 메모리 할당되고 함수가 종료될 때 메모리 초기화/사라짐
