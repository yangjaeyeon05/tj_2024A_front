// 1. 복습예제 : 특정 html( star div )에 '★'을 5개 출력하시오.
/*
    * DOM객체
 ...
 */
let html = ' ';              // 1. 데이터 구성
for( i = 1 ; i <= 5 ; i++ ) {// 2. 데이터를 1부터 5까지 1씩 증가하면서 하나씩 출력
    html += '★'    // ㅁ + 한자키 -> 8 -> ★;
    // 3. 누적합계 : 변수 += 값 , 변수 = 변수 + 값
}
// 4. 특정 HTML 마크업의 innerHTML속성에 변수값 대입하기.
document.querySelector('#star1').innerHTML = html;

// 2. 복습예제 : 특정 html( star div )에 총점 5점에서 고객평점을 출력하시오.
//  출력형식   : 만약에 고객평점이 3점이면 ★★★☆☆ 
let 고객평점 = 3 ;
let html2 = ' ';                // 1. 구성할 html 변수 선언
for( i = 1 ; i <= 5 ; i++ ){    // 2. 1부터 5까지 1씩 증가하면서 ☆출력
    if( i<= 고객평점 ){ html2 += '★' ; continue ; } // 3. 고객평점만큼 ★출력
    html2 += '☆'; // 아니면
}
document.querySelector('#star2').innerHTML = html2;

// 3. 복습예제 : 특정 html ( sheet div )에 좌석배열 상태에 따라 아래와 같이 출력하시오.
/*
    - 빈좌석 : x , 예약된좌석 : o
    - 2개씩 3줄 출력 하고, 빈좌석이면 배경색을 gray 표시, 예약된 좌석이면 red 표시
        ㅁㅁ
        ㅁㅁ
        ㅁㅁ
*/
