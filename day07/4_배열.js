/*
    ...
    - 배열 
        ...
        7. 배열내 특정 데이터 삭제
            1. 변수명.splice( 삭제할인덱스 , 1 )                   : 특정인덱스를 1개 삭제한다. ( 한칸씩 당겨진다. )
            2. 변수명.splice( 삽입할인덱스 , 0 , '새로운데이터' )   : 특정인덱스에 새로운데이터 삽입한다. ( 한칸씩 밀려진다. )

        8. 배열내 특정 데이터의 인덱스 찾기
            변수명.indexOf( 찾을 데이터 )                          : 만약에 찾을 데이터가 배열내 존재하면 위치/인덱스 반환, 없으면 -1
*/
...
console.log( 과일상자변수.indexOf('포도') );       // 3번째 존재 ==> 인덱스 : 2
    // 활용
let 삭제할인덱스 = 과일상자변수.indexOf('포도');    // 2 => 변환된 2를 '삭제할인덱스' 변수에 저장된다.
