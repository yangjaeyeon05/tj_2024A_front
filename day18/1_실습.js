...
function 제품등록(){
  ...
  // 1. 입력받은 값을 가져온다.
  let 제품명 = document.querySelector('#제품명').value;   console.log( 제품명 );
  let 가격 = document.querySelector('#가격').value*1;       console.log( 가격 );
  가격 = 가격.toLocaleString();
  // 2. 데이터를 가공( 객체화 )
  console.log(제품목록.length-1 );
  let 제품번호 = 제품목록.length != 0 ? 제품목록[제품목록.length-1].제품번호+1 : 1; // 현재 제품목록의 마지막 제품번호 +1 , 길이 1부터 /인덱스 0부터
  // 제품목록[제품목록.length-1] 배열의 마지막 인덱스의 .제품번호+1 속성 제품번호 +1 
  console.log( 제품번호 );
  let 제품 = { 제품번호 : 제품번호 , 제품명 : 제품명 , 가격 : 가격 };     console.log( 제품 );
  // 3. [유효성검사]
  // 4. 가공된 데이터(객체)를 저장(배열)한다.
  제품목록.push(제품);alert('등록성공');
...
// 2. 제품출력 함수 : 1. JS가 열렸을 때 2. 제품 등록/수정/삭제 했을 때
제품출력();
...
function 주문등록( 제품번호 ){  console.log( '주문등록()' + 제품번호 );
  ...
  let 주문번호 = 주문목록.length != 0 ? 주문목록[주문목록.length-1].주문번호 +1 : 1;  // 마지막 주문번호에 +1 , 마지막 주문번호 = 마지막 인덱스의 주문번호 = 배열.length-1
  let date = new Date();
...
// * 제품번호를 매개변수로 전달받고 해당 제품번호(매개변수)의 제품객체(정보:제품명,가격)를 찾아서 반환해주는 함수
function 제품조회(제품번호){
  ...
  for( let i = 0 ; i < 제품목록.length ; i++){
    ...
    if( 제품번호 == 제품목록[i].제품번호 ){
      ...
      return 찾은제품;  // 찾았으면 함수 종료하고 찾은 객체 (해당 함수를 호출한 곳으로)반환
    } // if end
  } // for end
}
...
function 자동주문(){ console.log('자동주문()');
  // * setInterval( 함수정의 또는 함수명 , 밀리초 ) 특정 주기마다 함수를 실행해주는 라이브러리함수
  setInterval( ()=>{ 
    // 주문할 제품번호를 난수 생성( 현재 제품목록에 존재하는 제품번호만 , 제품번호 1 ~ 마지막 제품번호 )
    let 끝제품번호 = 제품목록[제품목록.length-1].제품번호;    console.log(끝제품번호);  // 2
    // 난수생성
    let 제품번호 = parseInt((Math.random()*끝제품번호)+1);  console.log(제품번호);  //(Math.random()*끝번호)+시작번호
    // 난수 생성 후 주문등록 함수 호출
    주문등록(제품번호) 
  } , 3000 );    // setInterval( 함수호출이 아닌 함수를 호출하겠다고 **선언을 하는것, 밀리초 
}
...
// 6-3 : 주문목록을 브라우저(프로그램)의 localStorage 저장
// 6-4 : 주문목록을 브라우저(프로그램)의 localStorage 호출

  // Math.random() : 0~1
  // Math.random()+1 : 1~2
  // Math.random()+1*10 : 10~11미만
  // (Math.random()*10) : 0~10미만
  // (Math.random()*10)+1 : 1~11미만
  // (Math.random()*2)+1  : 1~3미만
